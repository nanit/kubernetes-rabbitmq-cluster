...
if [ "$1" = 'rabbitmq-server' ] && [ "$haveConfig" ]; then
  ...
  rabbitConfig+=(
    "{ log_levels, [{connection, error}, {mirorring, error}, {channel, error}, {federation, error}] }"
  )
...
if [ "$haveSslConfig" ] && [ -f "$combinedSsl" ]; then
  ...
  export RABBITMQ_SERVER_ADDITIONAL_ERL_ARGS="-rabbit log_levels [{connection,warning}]"
fi
